...
- **Backend Web Server**: Render.com
- **Communications Agent**: Render.com (Background Worker)
...
- Accounts on: Supabase, Render, Vercel/Netlify
...
3. **Set Environment Variables**
   ```
   FLASK_SECRET_KEY=<auto-generated or provide your own>
   FLASK_DEBUG=false
   AGENT_API_URL=https://calendar-agent-api.onrender.com
   SQL_CS=postgresql://[user]:[password]@[host]:[port]/[database]
   ```

4. **Deploy**
   - Click "Create Web Service"
   - Note your service URL (e.g., `https://calendar-backend-web.onrender.com`)
...
2. **Update Configuration**
   ...
   ```javascript
   const CONFIG = {
       WEB_SERVER_URL: 'https://calendar-backend-web.onrender.com',
       WEBSOCKET_URL: 'https://calendar-backend-web.onrender.com',
       API_BASE: '/api',
       ENABLE_DEBUG: false
   };
   ```
...
3. **Web Server Status**
   ```bash
   curl https://calendar-backend-web.onrender.com/api/status
   ```
...
### Render Dashboard
- View logs, metrics, and deployments for all services
...
- **Render**: Spins down after 15 min inactivity (web services)
