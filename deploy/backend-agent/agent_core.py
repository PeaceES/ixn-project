...
from services.server_client_db import CalendarClient
...
TEMPERATURE = 0.1
TOP_P = 0.1
INSTRUCTIONS_FILE = "./assets/general_instructions.txt"
...
class CalendarAgentCore:
        ...
        try:
            ...
            def find_entity_email(entity_name: str):
                if not entity_name:
                    return None
                import os, json
                # Try to load from assets directory first (for deployment)
                org_path = os.path.join(os.path.dirname(__file__), 'assets/org_structure.json')
    ...
    async def initialize_agent(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Initialize project client with AAD authentication
            # For local development: use 'az login' 
            # For CI/Docker: set AZURE_CLIENT_ID, AZURE_TENANT_ID, AZURE_CLIENT_SECRET
            logger.info("[AgentCore] Using DefaultAzureCredential for AAD authentication")
            logger.info("[AgentCore] Ensure you're logged in with 'az login' or have service principal env vars set")
