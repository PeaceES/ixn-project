...
# Global storage
rooms_data = {}
group_calendars_data = {}
calendars_data = {}  # Added missing calendars_data
events_data = {"events": []}
user_directory = {}
...
GROUP_CALENDARS_FILE = os.path.join(DATA_DIR, "group_calendars.json")
...
async def load_group_calendars():
    """Load group calendars from JSON file."""
    global group_calendars_data
    try:
        with open(GROUP_CALENDARS_FILE, 'r') as f:
            group_calendars_data = json.load(f)
        print(f"✅ Loaded {len(group_calendars_data.get('group_calendars', []))} group calendars from {GROUP_CALENDARS_FILE}")
    except FileNotFoundError:
        print(f"⚠️ {GROUP_CALENDARS_FILE} not found, using empty group calendars list")
        group_calendars_data = {"group_calendars": []}
    except Exception as e:
        print(f"❌ Error loading group calendars: {e}")
        group_calendars_data = {"group_calendars": []}


async def load_calendars():
    """Load regular calendars from JSON file."""
    global calendars_data
    try:
        # For now, use group calendars as regular calendars
        await load_group_calendars()
        calendars_data = group_calendars_data
        print(f"✅ Loaded {len(calendars_data.get('calendars', []))} calendars")
    except Exception as e:
...
def validate_group_exists(group_id: str) -> tuple[bool, str, dict]:
    """Validate if group exists and return group info."""
    for group in group_calendars_data.get("group_calendars", []):
        if group["id"] == group_id:
            return True, "Group found", group
...
def validate_user_in_group(user_id: str, group_id: str, group_info: dict) -> tuple[bool, str]:
...
def validate_group_room_permission(group_id: str, room_id: str, group_info: dict) -> tuple[bool, str]:
    """Validate if group has permission to book the room."""
    allowed_rooms = group_info.get("allowed_rooms", [])
    if "all" in allowed_rooms or room_id in allowed_rooms:
        return True, "Group has permission to book this room"
    
    return False, f"Group '{group_id}' does not have permission to book room '{room_id}'"


def validate_user_permissions(user_id: str, calendar_id: str) -> tuple[bool, str]:
    """Validate if user has permission to access the calendar."""
    user_exists, user_msg, user_info = validate_user_exists(user_id)
    if not user_exists:
        return False, user_msg
    
    # For group-based system, check if user is member of any group
    for group in group_calendars_data.get("group_calendars", []):
        if user_id in group.get("members", []):
            return True, "User has group access"
    
    return False, f"User '{user_id}' has no group memberships"


def validate_calendar_exists(calendar_id: str) -> tuple[bool, str, dict]:
    """Validate if calendar exists and return calendar info."""
    # Check in group calendars first
    for group in group_calendars_data.get("group_calendars", []):
        if group["id"] == calendar_id:
            return True, "Calendar found", group
    
    # Check in regular calendars
