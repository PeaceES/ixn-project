#!/usr/bin/env python3
"""
Simple test script to validate basic agent functionality without complex toolsets.
This tests the most basic agent functionality to isolate the issue.
"""
...
async def test_basic_agent():
    ...
    print(f"{tc.CYAN}Testing Basic Agent (No Tools)...{tc.RESET}")
    ...
    try:
        ...
        agent = await project_client.agents.create_agent(
            ...
            name=AGENT_NAME,
            instructions="You are a helpful assistant. Respond to user messages politely and helpfully.",
...
if __name__ == "__main__":
    print(f"{tc.CYAN}Starting basic agent test...{tc.RESET}")
    result = asyncio.run(test_basic_agent())
    if result:
        print(f"{tc.GREEN}🎉 Basic agent test PASSED! The Azure AI connection works.{tc.RESET}")
    else:
        print(f"{tc.RED}💥 Basic agent test FAILED. There's a fundamental issue.{tc.RESET}")
